function [means,diffs,meanDiff,CR,linFit] = BlandAltman(var1, var2, flag)
    ...
    if flag ~= 0
        ...
        if flag > 1
            plot(means, ones(1,length(means)).*CR(1),'b--','linewidth',2); hold on %%%plot the upper CR,
            plot(means, ones(1,length(means)).*CR(2),'b--','linewidth',2); %%%plot the lower CR
            plot(means,zeros(1,length(means)),'k','linewidth',2); %%%plot zero
            ylim([1.2*CR(2) 1.2*CR(1)])
        end
        if flag > 2
            plot(means, means.*linFit(1)+linFit(2),'k--'); %%%plot the linear fit
        end
        plot(means,diffs,'o','MarkerEdgeColor','r','MarkerFaceColor','r','MarkerSize',7)
